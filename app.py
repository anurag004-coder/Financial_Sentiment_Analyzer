...
# --- 5. Batch Processing (Auto-Loaded CSV) ---
...
# Define the file path (assumes it is in the same folder as this app.py file)
file_path = "financial_phrasebank.csv"

# Check if the file exists in the GitHub repository
if os.path.exists(file_path):
    # Read the dataset (Your specific CSV has no headers, so we name them here)
    df = pd.read_csv(file_path, header=None, names=["True_Sentiment", "Sentence"])
